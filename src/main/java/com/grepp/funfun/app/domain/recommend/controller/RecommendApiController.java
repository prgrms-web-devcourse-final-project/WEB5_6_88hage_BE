...
public class RecommendApiController {
    ...
    public CompletableFuture<ResponseEntity<ApiResponse<String>>> chatBotCloseAndSummary(
        ...
        String prompt = chatBotService.recommendCategoryPrompt(conversation, request.getEventType());
...
//                                                   request.getEndTime()); // 여가시간
        ...
        StartAndDuringDTO userTime = chatBotService.getTime(request.getStartTime(),
                                                            request.getEndTime());
        String prompt = chatBotService.getPrompt(summary, agePrompt, request.getAddress());
        CompletableFuture<RecommendContentDTO> contentFuture = new CompletableFuture<>();

        aiRequestQueue.addRequest(
            new AiRequestQueue.AiRequestTask<>(() -> {
                return dynamicService.chatBotRecommendContent(prompt, userTime.startDate(), userTime.endDate());
            }, contentFuture)
        );
        ...
        return contentFuture
            .thenApplyAsync(contents -> {
                ...
                List<ContentWithReasonDTO> recommendContents = contentService.findByIds(
                    recommendIds);
    ...
    public CompletableFuture<ResponseEntity<ApiResponse<RecommendGroupResponse>>> chatBotRecommendGroup(
        ...
        aiRequestQueue.addRequest(
            new AiRequestQueue.AiRequestTask<>(() -> {
                return dynamicService.chatBotRecommendGroup(prompt, userTime.startDate(), userTime.endDate()); // inmemory
            }, groupFuture)
        );
        ...
        return groupFuture
            .thenApplyAsync(groups -> {
                if(groups == null || groups.group() == null){
                    return ResponseEntity.ok(ApiResponse.success(RecommendGroupResponse.builder().groups(List.of()).build()));
                }
    ...
    public CompletableFuture<ResponseEntity<ApiResponse<RecommendTwoListResponse>>> quickRecommendContent(
        @Valid @RequestBody RecommendRequest request, Authentication authentication) {
        String email = authentication.getName();
        String preference = userService.getUserPreferenceDescription(email, "CONTENT");  // 나중에 DTO로 묶어서 한번에 추출할지 고려해보기
        String agePrompt = userService.getUserAgePromptByEmail(email);
        String userAddress = userService.getUserAddress(email);
...
//                                                   request.getEndTime()); // 여가시간
        ...
        String promptForEvent =
            agePrompt + preference + "나는 지금 " + request.getAddress() + "에 있어"
        ...
        aiRequestQueue.addRequest(
            new AiRequestQueue.AiRequestTask<>(() -> {
                return dynamicService.quickRecommendEvent(promptForEvent, userTime.startDate(), userTime.endDate());
            }, contentFuture)
        );
        ...
        aiRequestQueue.addRequest(
            new AiRequestQueue.AiRequestTask<>(() -> {
                return dynamicService.quickRecommendPlace(promptForPlace, finalUserAddress);
            }, placeFuture)
        );
        ...
        return contentFuture
            ...
            .thenApplyAsync(contents -> {
                if(contents == null || contents.event() == null){
                    List<ContentWithReasonDTO> result = List.of();
                    return result;
                }
                List<RecommendDTO> recommendList = contents.event();
                Map<Long, String> reasonMap = recommendList.stream()
                ...
                List<ContentWithReasonDTO> recommendContents = contentService.findByIds(
                    recommendIds);
                recommendContents.forEach(
                    content -> content.setReason(reasonMap.get(content.getId())));
            ...
            .thenCombineAsync(placeFuture.thenApplyAsync(place -> {
                if(place == null || place.event() == null){
                    List<ContentWithReasonDTO> result = List.of();
                    return result;
                }
                ...
                log.info("장소 추천 결과 ==================");
                recommendPlaceList.forEach(
                    dto -> log.info("id: {}, 추천이유 : {}", dto.id(), dto.reason()));

                List<ContentWithReasonDTO> recommendPlaces = contentService.findByIds(
                    recommendPlaceIds);
                recommendPlaces.forEach(
                    content -> content.setReason(reasonPlaceMap.get(content.getId())));
    ...
    public CompletableFuture<ResponseEntity<ApiResponse<RecommendGroupResponse>>> quickRecommendGroup(
        ...
        aiRequestQueue.addRequest(
            new AiRequestQueue.AiRequestTask<>(() -> {
                return dynamicService.quickRecommendGroup(prompt, userTime.startDate(), userTime.endDate());
            }, groupFuture)
        );
